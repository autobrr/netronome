  ...
  return (
    <div className="bg-gray-50/95 dark:bg-gray-850/95 rounded-xl p-6 shadow-lg border border-gray-200 dark:border-gray-800">
        ...
        <div className="flex items-center gap-2">
          {onCreateMonitor && (
            <motion.button
              ...
              title={t('traceroute.createMonitorDesc')}
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
              <SignalIcon className="w-3 h-3" />
              <span>{t('traceroute.createMonitor')}</span>
            </motion.button>
          )}
          <motion.button
      ...
      <p className="text-gray-600 dark:text-gray-400 text-sm mb-6">
        ...
        {" • "}
        {results.totalHops} {t('traceroute.hop')}s •{" "}
        {results.complete ? t('traceroute.complete') : "Incomplete"}
      </p>
